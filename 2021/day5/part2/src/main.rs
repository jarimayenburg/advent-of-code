...
impl Vent {
    ...
    fn line(&self) -> Vec<Point> {
        if self.start.x == self.end.x {
            ...
            let x_i: i32 = if self.end.x > self.start.x { 1 } else { -1 };
            let y_i: i32 = if self.end.y > self.start.y { 1 } else { -1 };

            let mut x = self.start.x as i32;
            let mut y = self.start.y as i32;

            let len_x = (self.start.x as i32 - self.end.x as i32).abs();
...
fn main() {
    ...
    let vents = Vent::from_input(&input);
