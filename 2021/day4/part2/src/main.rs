...
fn main() {
    ...
    let mut cards = build_cards(&raw_inputs);

    let mut called: Vec<u32> = Vec::new();

    let mut last_won_score: u32 = 0;

    for call in &calls {
        called.push(*call);

        let mut cards_next_round: Vec<Vec<Vec<u32>>> = Vec::new();
        for i in 0..cards.len() {
            let card = &cards[i];

            if has_won(&card, &called) {
                last_won_score = calc_score(&card, &called, *call);
            }
            else {
                cards_next_round.push(card.to_vec());
            }
        }

        cards = cards_next_round;
    }

    println!("The last card to win had score {}", last_won_score);
}
